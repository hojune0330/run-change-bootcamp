...
- 공개 프리뷰의 기준선은 배포된 `main`입니다. 현재 작업 브랜치는 `origin/main`보다 19개 커밋 앞선 미공개 로컬 브랜치이며, 아직 Pages에 배포되지 않았습니다. 이 통합 브랜치의 타입·테스트·PostgreSQL 17.10·브라우저 전체 gate는 아직 실행 전이며 Todo 3에서 확인합니다.
...
- 위 공개 프리뷰에는 배포된 `main`만 반영되며, 현재 미공개 로컬 브랜치의 변경 사항은 포함되지 않습니다.
...
|---|---|---|
| 정적 계약 확인(static contract checked) | 타입, 단위/통합 테스트, SQL/RLS 계약을 로컬에서 검사 | 공개 `main`/기존 POC 기록에서 확인; 현재 통합 브랜치 gate 대기 |
| 로컬 런타임(local runtime) | Node 22 브라우저 프리뷰와 PostgreSQL 17.10 보안 시나리오를 로컬 프로세스로 실행 | 공개 `main`/기존 POC 기준; 현재 통합 브랜치 gate 대기 |
...
통합 브랜치의 전체 gate 결과는 아직 없으며, 실제 자격 증명·이메일 전달이나 호스팅 Supabase를
검증했다는 뜻도 아닙니다. 파일럿 화면에는 운영 데이터 연결이 준비 단계라고 명시합니다.
